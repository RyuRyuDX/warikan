...
- **Site URL**: `http://localhost:3000`（本番デプロイ後は Vercel URL に変更）
- **Redirect URLs**: `http://localhost:3000/auth/callback` を追加（本番URLも追加）
...
http://localhost:3000 を開いて、メールアドレスでログイン → 表示名を入れて使い始められます。
...
### ログインリンクが届かない
- 迷惑メールフォルダ確認
- Supabase の **Authentication → Email Templates** が日本語化されているか確認（任意）
